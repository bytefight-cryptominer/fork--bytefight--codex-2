...
class PlayerController:
    ...
    def _hill_status(self, board, target, parity):
        my = 0
        opp = 0
        for r, c in target:
        ...
        total = len(target)
        threshold = math.ceil(total * GameConstants.HILL_CONTROL_THRESHOLD)
        my_captured = my >= threshold and my > opp
        opp_captured = opp >= threshold and opp > my
        return my, opp, threshold, my_captured, opp_captured

    def _collect_target_positions(self, board, target, me, parity):
        goals = []
        approaches = set()
        for r, c in target:
            loc = Location(r, c)
            ...
            for d in Direction.cardinals():
                ...
                if board.oob(nl) or board.cells[nl.r][nl.c].is_wall:
                    continue
                if (nl.r, nl.c) not in target:
                    approaches.add(nl)

        ranked = []
        for loc in goals:
            dist = abs(me.loc.r - loc.r) + abs(me.loc.c - loc.c)
            ranked.append((0, dist, loc))
        for loc in approaches:
            cell = board.cells[loc.r][loc.c]
            risk_tier = 0 if cell.owner_parity == parity else 1 if cell.owner_parity == 0 else 2
            dist = abs(me.loc.r - loc.r) + abs(me.loc.c - loc.c)
            ranked.append((1 + risk_tier, dist, loc))
        ranked.sort(key=lambda item: (item[0], item[1], item[2].r, item[2].c))
        return [loc for _, _, loc in ranked[:6]]

    def _estimate_opponent_danger(self, board, opp, parity):
        danger = set()
        if not opp:
            return danger
        ...
        max_moves = self._max_regular_moves(opp_effective_stamina)
        danger.update(self._shortest_distance_map(board, opp_r, opp_c, max_moves).keys())

        current_cell = board.cells[opp_r][opp_c]
        if current_cell.beacon_parity == -parity:
            beacon_budget = max(1, max_moves - 1)
            for r, row in enumerate(board.cells):
                for c, cell in enumerate(row):
                    if cell.beacon_parity != -parity:
                        continue
                    danger.add((r, c))
                    danger.update(self._shortest_distance_map(board, r, c, beacon_budget).keys())
        return danger

    def _build_plan_sequences(self, board, me, parity, path_dirs, target):
        final_loc = me.loc
        for move_dir in path_dirs:
            final_loc = final_loc + move_dir

        pre_targets = self._paintable(board, me.loc, parity, target)[:2]
        post_targets = self._paintable(board, final_loc, parity, target)[:2]
        ...
        if path_dirs:
            ...
            for move_dir in path_dirs:
                cur = cur + move_dir
                base_moves.append(Action.Move(move_dir))
            move_variants = [base_moves]

            final_cell = board.cells[final_loc.r][final_loc.c]
            if final_cell.owner_parity == -parity and (
                (final_loc.r, final_loc.c) in target or any(
                    not board.oob(final_loc + d) and ((final_loc + d).r, (final_loc + d).c) in target
                    for d in Direction.cardinals()
                )
            ):
                erase_moves = list(base_moves)
                erase_moves[-1] = Action.Move(path_dirs[-1], move_type=MoveType.ERASE)
                move_variants.append(erase_moves)

        pre_variants = [[]]
        if pre_targets:
            pre_variants.append([Action.Paint(pre_targets[0])])

        post_variants = [[]]
        if post_targets:
            post_variants.append([Action.Paint(post_targets[0])])
            if len(post_targets) > 1:
                post_variants.append([Action.Paint(post_targets[0]), Action.Paint(post_targets[1])])

        sequences = []
        for pre in pre_variants:
            for moves in move_variants:
                for post in post_variants:
                    sequences.append(pre + moves + post)
        return sequences

    def _score_hill_plan(self, board, future, me, parity, target, goal_loc, danger_cells):
        before_my, before_opp, _, before_captured, before_opp_captured = self._hill_status(board, target, parity)
        after_my, after_opp, _, after_captured, after_opp_captured = self._hill_status(future, target, parity)

        before_me = me
        after_me = future.get_player(parity)
        before_goal_dist = abs(before_me.loc.r - goal_loc.r) + abs(before_me.loc.c - goal_loc.c)
        after_goal_dist = min(
            abs(after_me.loc.r - r) + abs(after_me.loc.c - c)
            for r, c in target
        )

        territory_gain = future.get_territory_count(parity) - board.get_territory_count(parity)
        opp_territory_loss = board.get_territory_count(-parity) - future.get_territory_count(-parity)
        hill_gain = after_my - before_my
        hill_denial = before_opp - after_opp
        progress = before_goal_dist - after_goal_dist

        opp_dist = abs(after_me.loc.r - future.get_player(-parity).loc.r) + abs(after_me.loc.c - future.get_player(-parity).loc.c)
        stamina_weight = 1.2 if opp_dist <= self.safe_dist + 1 else 0.45

        score = progress * 24.0
        score += hill_gain * 70.0
        score += hill_denial * 60.0
        score += territory_gain * 4.0
        score += opp_territory_loss * 3.0
        score += after_me.stamina * stamina_weight

        if not before_captured and after_captured:
            score += 260.0
        if before_opp_captured and not after_opp_captured:
            score += 220.0

        end_loc = after_me.loc
        secure_end = future.cells[end_loc.r][end_loc.c].owner_parity == parity
        if (end_loc.r, end_loc.c) in danger_cells and not secure_end:
            score -= 500.0

        meaningful = (
            progress > 0 or hill_gain > 0 or hill_denial > 0 or
            (not before_captured and after_captured) or
            (before_opp_captured and not after_opp_captured)
        )
        return score, meaningful, secure_end

    def _plan_hill_turn(self, board, me, opp, parity, target, near_opp, time_budget):
        if not target or time_budget < 60 or me.stamina < 25:
            return None

        nearest_target = min(abs(me.loc.r - r) + abs(me.loc.c - c) for r, c in target)
        if nearest_target > 6:
            return None

        max_depth = min(3, self._max_regular_moves(me.stamina))
        if max_depth <= 0:
            return None

        target_positions = self._collect_target_positions(board, target, me, parity)
        if not target_positions:
            return None

        danger_cells = self._estimate_opponent_danger(board, opp, parity)
        best_actions = None
        best_score = -999999.0
        seen = set()

        for goal_loc in target_positions:
            ...
            for depth in range(1, len(path) + 1):
                path_prefix = path[:depth]
                for actions in self._build_plan_sequences(board, me, parity, path_prefix, target):
                    sig = tuple(str(action) for action in actions)
                    if sig in seen:
                        continue
                    seen.add(sig)
                    future, ok = board.forecast_turn(parity, actions)
                    if not ok:
                        continue
                    score, meaningful, secure_end = self._score_hill_plan(
                        board, future, me, parity, target, goal_loc, danger_cells
                    )
                    if not meaningful:
                        continue
                    if score > best_score:
        ...
        if best_actions and best_score >= 45.0:
            return best_actions
        return None
    ...
    def play(
        ...
        tl = time_left()
        planned_actions = self._plan_hill_turn(board, me, opp, player_parity, target, near_opp, tl)
        if planned_actions:
